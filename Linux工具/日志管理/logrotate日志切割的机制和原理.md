...
最近整理nginx日志，用了一个类Unix系统上的古老工具——logrotate，发现意外的好用。
...
logrotate是**怎么做到滚动日志时不影响程序正常的日志输出呢？**
logrotate提供了两种解决方案。
...
![](attachments/Pasted%20image%2020240723104350.png)

目录也是文件，文件里存着文件名和对应的inode编号。
通过这个inode编号可以查到文件的元数据和文件内容。文件的元数据有引用计数、操作权限、拥有者ID、创建时间、最后修改时间等等。文件名并不在元数据里而是在目录文件中。因此文件改名、移动，都不会修改文件，而是修改目录文件。

### 进程打开文件
借《UNIX环境高级编程》里的图说一下进程打开文件的机制。

![](attachments/Pasted%20image%2020240723104508.png)

进程每新打开一个文件，系统会分配一个新的文件描述符给这个文件。文件描述符对应着一个文件表。表里面存着文件的状态信息（`O_APPEND`/`O_CREAT`/`O_DIRECT`...）、当前文件位置和文件的inode信息。文件表中存的是inode信息而不是文件路径，所以文件路径发生改变不会影响文件操作。

系统会为每个进程创建独立的文件描述符和文件表，不同进程是不会共用同一个文件表。正因为如此，不同进程可以同时用不同的状态操作同一个文件的不同位置。

## 方案1：create

默认方案没有名字，姑且叫它create吧。因为这个方案会创建一个新的日志文件给程序输出日志。
...
什么方式通知程序我重新打开日志呢？
> 简单粗暴的方法是杀死进程重新打开。很多场景这种作法会影响在线的服务，于是有些程序提供了重新打开日志的接口，比如可以通过信号通知nginx。各种IPC方式都可以，前提是程序自身要支持这个功能。
> 注意：这里杀死进程指的是杀死syslogd进程，因为是syslogd进程打印日志到指定的文件，而不是比如nginx这种调用syslog接口的进程。
...
## 总结
copytruncate是个配置项，与create配置项是互斥的。

create 方案是创建一个新的同名的文件，杀死syslogd进程重新基于文件名打开文件，操作前后同名的文件的inode发生了改变。

copytruncate方案则是将正在输出的日志拷(copy)一份出来，再清空(trucate)原来的日志。期间文件名以及inode都没有发生改变。
