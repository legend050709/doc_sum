...
![](attachments/Pasted%20image%2020231113164532.png)

而且在用 wireshark 工具抓包的时候，我们也会常看到 TCP 挥手过程是三次，而不是四次，如下图：
![](attachments/Pasted%20image%2020231113164542.png)
...
## 流程
TCP 四次挥手的过程如下：
![](attachments/Pasted%20image%2020231113164606.png)

具体过程：

- 客户端主动调用关闭连接的函数，于是就会**发送 FIN 报文，这个 FIN 报文代表（客户端）不会再发送数据了**，进入 FIN_WAIT_1 状态；
...
**注意：如果有多进程/多线程共享同一个 `socket`，如果有一个进程调用了 `close` 关闭只是让 `socket` 引用计数 `-1`，并不会导致 `socket` 不可用，同时也不会发出 `FIN` 报文，其他进程还是可以正常读写该 `socket`，直到引用计数变为 `0`，才会发出 `FIN` 报文**。
...
![](attachments/Pasted%20image%2020231113165516.png)
...
## shutdown 函数
![](attachments/Pasted%20image%2020231114202453.png)
...
![](attachments/Pasted%20image%2020231113165655.png)
...
![](attachments/Pasted%20image%2020231113165908.png)
> 注：因为 TCP 延迟确认机制是默认开启的，所以导致我们抓包时，看见三次挥手的次数比四次挥手还多。

# 其他
## FIN_WAIT1 状态的优化
主动方发送 FIN 报文后，连接就处于 FIN_WAIT1 状态，正常情况下，如果能及时收到被动方的 ACK，则会很快变为 FIN_WAIT2 状态。

但是当迟迟收不到对方返回的 ACK 时，连接就会一直处于 FIN_WAIT1 状态。此时，内核会定时重发 FIN 报文，其中重复次数由 tcp_orphan_retries 参数控制（注意，orphan 虽然是孤儿的意思，该参数却不只对孤儿连接有效，事实上，它对所有 FIN_WAIT1 状态下的连接都有效），默认值是 0。

![](attachments/Pasted%20image%2020231114202735.png)
这 0 表示几次？实际上当为 0 时，特指 8 次，从下面的内核源码可知：
![](attachments/Pasted%20image%2020231114202810.png)
...
![](attachments/Pasted%20image%2020231113170113.png)

### 延迟时间
延迟等待的时间是在 Linux 内核中定义的，如下图：
![](attachments/Pasted%20image%2020231113170148.png)
关键就需要 HZ 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 1000，如下图：
![](attachments/Pasted%20image%2020231113170203.png)
知道了 HZ 的大小，那么就可以算出：
...
编译服务端和客户端的代码：
![](attachments/Pasted%20image%2020231113170405.png)

先启用服务端：
![](attachments/Pasted%20image%2020231113170416.png)
...
![](attachments/Pasted%20image%2020231113170454.png)
此时，服务端的输出：
![](attachments/Pasted%20image%2020231113170501.png)
接下来，我们来看看抓包的结果。
![](attachments/Pasted%20image%2020231113170511.png)
可以看到，TCP 挥手次数是 3 次。
...
![](attachments/Pasted%20image%2020231113170849.png)
编译好服务端代码后，就开始运行服务端和客户端的代码，同时用 tcpdump 进行抓包。

抓包的结果如下，可以看到是四次挥手。
![](attachments/Pasted%20image%2020231113170857.png)
...
https://zhuanlan.zhihu.com/p/146752547
```