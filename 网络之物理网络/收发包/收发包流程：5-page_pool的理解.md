...
- **缓存复用**：驱动不再每次向系统申请新页（从全局的 Buddy System 申请，需要加锁），而是从 `page_pool` 中获取“用过但已回收”的页。
- **DMA 映射持久化**：页面在加入 pool 时已经做好了 DMA 映射（或快速映射），取出时无需重新映射，极大减少 IOMMU 操作。
- **锁无关（Lockless）**：利用 per-CPU 缓存（ptr-ring）技术，绝大多数分配/释放操作无需加锁，极大提升并发性能。
- **NUMA 感知**：确保分配的内存页位于网卡所在的 NUMA 节点，减少跨节点访问延迟。
