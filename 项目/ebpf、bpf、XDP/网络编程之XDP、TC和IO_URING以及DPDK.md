...
也正因为如此，TC和硬件基本没有半毛钱关系了，这和XDP是一个比较明显的不同。
 ...
 由于位置的不同，所处的协议栈的层次不同，因此其对数据的处理就有所不同，XDP位于底层，数据更原始完整，可以进行原始报文的修改控制；而TC处于上层，但可以使用更强大的数据结构处理更复杂的修改需求。
...
DPDK在大容量高吞吐等场景有优势。XDP在云原生等场景有优势。
XDP相较与DPDK来说，具有以下**优点：**

- DPDK会独占CPU资源且需要大页内存。XDP无需专门硬件，无需大页内存，无需独占CPU等资源，任何有Linux驱动的网卡都可以支持，无需引入第三方代码库。
