...
解开封装后，查找路由，如果发现   dip（VIP）的出口（即虚拟隧道口） 和 sip(clinent-ip)的出口（ local-ip所在的 物理口）不一样，就会基于 rp_filter 的取值，决定是否丢弃。


**设置**
注意 `Ip Tunnel`模式下需要将RS上虚拟口的`rp_filter`参数配为0 或者 2，否则无法正常工作，因为RS是在物理网卡收到请求，但是VIP是绑在虚拟网卡tunl0上的。
