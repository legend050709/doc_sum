...
![](attachments/Pasted%20image%2020240606175243.png)
...
kill -SIGHUP PID
```
> 注：在上面所列的信号列表中，KILL和STOP这两个信号无法被捕捉。一般来说，在设置信号陷阱时，只会考虑HUP、INT、QUIT、TERM这4个会终止、中断进程的信号。
...
## SIGPIPE信号
该信号对应的数值为 13。
该信号意味着管道错误，通常在进程间通信产生，比如采用`FIFO`(管道)通信的两个进程，==读管道没打开或者意外终止仍继续往管道写==，写进程就会收到 `SIGPIPE` 信号。
