...
```
# 概述
信号是异步的，它会在程序的任何地方发生。由此程序正常的执行路径被打破，去执行信号处理函数。**一般情况下，进程正在执行某个系统调用，那么在该系统调用返回前信号是不会被传递的。但慢慢系统调用除外，这些系统调用都会返回-1，errno置为EINTR**。当系统调用被中断时，我们可以选择使用循环再次调用，或者设置重启该系统调用。
