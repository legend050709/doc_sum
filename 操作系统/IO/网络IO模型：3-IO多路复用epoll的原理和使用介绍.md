...
因为水平触发时如果系统中有大量你不需要读写的就绪文件描述符，而它们每次都会返回，这样会大大降低处理程序检索自己关心的就绪文件描述符的效率；而边缘触发不会充斥大量你不关心的就绪文件描述符，可以减少 `epoll_wait` 的系统调用次数，系统调用也是有一定的开销的的，毕竟也存在上下文的切换。
...
##### 在 ET 模式下 阻塞 I/O 的致命问题

#### `unblock listen-fd`在ET模式下的连接数和事件个数以及accept成功个数的关系
...
分析：因为还是会缺乏事件，去触发accept读取连接。
之前说==在同一个线程中==，边缘模式，不循环调用`accept`，会丢事件，进行不会去触发`accept`, 实际上并不会丢连接，连接还是在全连接队列中「不考虑全连接队列满的情况」。
...
平时不要把该描述符放进`epoll`结构体中，当需要写该fd的时候，调用`epoll_ctl`把fd加入`epoll`里监听，可写的时候就往里写，写完再次调用`epoll_ctl`把fd移出`epoll`。
> 注：**这种方式的缺点是，即使发送很少的数据，也要把socket加入epoll，写完后在移出epoll，有一定操作代价。**

改进一下就是：平时不要把该描述符放进`epoll`结构体中，需要写的时候调用`write`或者`send`写数据，如果返回值是`EAGAIN`（写缓冲区满了），那么这时候才执行第一种方法的步骤。
...
- 对于读写的sockfd「非listen的 sockfd， 比如 accept 产生的sockfd 或者 client的 connect fd」，边缘触发模式下，必须使用非阻塞IO，并要一次性全部读写完数据。
...
#### 解决
从 `Linux 4.5+` 开始出现的**LT水平触发**模式新增的 **`EPOLLEXCLUSIVE` 独占**标志，这个标志会保证一个事件只有一个 `epoll_wait`会被唤醒，避免了 “惊群效应”，并且可以在多个 CPU 之间很好的水平扩展。
...
此时本端会触发事件如下：
EPOLLRDHUP （需要主动在`epoll_ctl`时加入该`events`） +  EPOLLIN  + EPOLLOUT
...
> EPOLLERR + EPOLLHUP + EPOLLIN + EPOLLOUT + EPOLLRDHUP （需要主动在`epoll_ctl`时加入该`events`）;
...
> 注：`signal(SIGPIPE, SIG_IGN)`; `Broker pipie`这个异常，说到底是应用层没有对相应的fd在收到对端关闭通知时，作正确的处理所致，它并不是`tcp/ip`通讯层面的问题。
...
因此，为了防止异常情况发生， 还是建议 在 `close` 之前，先通过`epoll_ctl`将 `fd`从 `epollfd` 中删除。
