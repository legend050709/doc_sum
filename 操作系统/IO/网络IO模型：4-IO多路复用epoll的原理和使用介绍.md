...
ET（边缘触发）的核心是：**检测缓冲区状态的"变化沿"（edge）**。
...
时刻1: [ 未读数据 A ]         ← 已有可读状态，ET 已触发过（假设未读完）
时刻2: [ 未读数据 A | 新数据 B ] ← 新数据到达，缓冲区内容发生变化 → ET 再次触发
```
内核在新数据写入 socket 接收缓冲区时，会产生一次新的"边沿"事件，epoll ET 会捕获到这次变化并**重新通知**。

![](attachments/Pasted%20image%2020260429145611.png)
...
（5.1）监听了`EPOLLIN` 事件时，无论之前接收缓冲区中是否存在未读取完的数据，只要有新数据（收到fin也算是新数据到达）到达，都会产生`EPOLLIN` 事件。
...
#### 分析
`epoll`在`ET`边缘模式下，由于**每次来的消息如果你不一次性处理完的话，下一次就不通知你了**。
...
**一颗红黑树，一张准备就绪句柄链表（ready list），少量的内核cache**，就帮我们解决了大并发下的socket处理问题.

- 执行epoll_create() 时，创建了红黑树和就绪链表（ready list）;
> 注：红黑树来管理**注册fd**，就绪列表来收集所有**就绪fd**;
...
这个特殊的epfd描述符就是epoll实例的句柄，后面的两个接口都以它为中心。
同时也会创建红黑树和就绪列表，**红黑树来管理注册fd，就绪列表来收集所有就绪fd**。
...
此时注册 `EPOLLOUT` 事件可能会触发问题。因此，==尽量避免在未连接时（即connect之前）监听可写事件==。
...
#### EPOLLRDHUP
##### 背景
相信很多时候，大家都是通过检测 `read/recv` 返回 0 来判断对端是否关闭了连接，如果返回 0，我们通常也会 close 该连接。
这没有问题，但在很多场景下有个缺陷：**FIN 报文和普通数据报文一样，也是需要在缓冲区中排队的，只有当 read 读取到 FIN 以后才会返回 0；而且 FIN 报文无法和数据同时被读取，也就是说，必须将数据 read 完毕后，再调用一次 read 才能读取到 FIN 并返回 0** 。
这也是为什么在网络读取时需要将 read 放在循环中的原因之一，不仅是为了将数据读取完整，也是为了能够读到 FIN 报文。
...
(1)  每个进程可以 `bind/listen` 相同的 `IP/PORT`，相当于==每个进程/线程拥有独立的 `listen socket` 的完全队列（全连接队列），避免了共享 `listen socket` 的资源争抢==，提升了并发的吞吐。
...
## select/poll
### select的流程
select 实现多路复用的方式是，将已连接的 Socket 都放到一个文件描述符集合（FD_SET），然后调用 select 函数将文件描述符集合拷贝到内核里，让内核来检查是否有网络事件产生，检查的方式很粗暴，就是通过遍历文件描述符集合（FD_SET）的方式，当检查到有事件产生后，将此 Socket 标记为可读或可写， 接着再把整个文件描述符集合（FD_SET）拷贝回用户态里，然后用户态还需要再通过遍历文件描述符集合（FD_SET）的方法找到可读或可写的 Socket，然后再对其处理。

### select：两次遍历fdset，两次拷贝fdset
...
poll 不再用 BitsMap 来存储所关注的文件描述符，取而代之用动态数组，以链表形式来组织，突破了 select 的文件描述符个数限制，当然还会受到**系统文件描述符（系统级、用户级、进程级的fd个数）**限制。
...
- epoll通过红黑树描述，最大可以打开文件的数目（有系统级、用户级、进程级的三层限制），可以通过命令`ulimit -n number`修改，仅对当前用户（当前终端）有效。
...
即：epoll 使用事件驱动的机制，内核里维护了一个链表来记录就绪事件，**当某个 socket 有事件发生时，通过回调函数内核会将其加入到这个就绪事件列表中「通过回调函数，没有一个遍历查找的过程」**；当用户调用 `epoll_wait()` 函数时，只会返回有事件发生的文件描述符的个数，不需要像 select/poll 那样轮询扫描整个 socket 集合，大大提高了检测的效率。
...
**epoll 则将整个文件描述符集合维护在内核态，每次添加文件描述符的时候都需要执行一个系统调用（epoll_ctl_add）。系统调用的开销是很大的，而且在有很多短期活跃连接的情况下，由于这些大量的系统调用开销，epoll 可能会慢于 select 和 poll**。

- select 和 poll 的动作基本一致，只是 poll 采用链表的方式来存储 fd，而 select 采用 fd 标注位（bitsmap）来存放，所以 select 会受到最大连接数的限制而 poll 不会。epoll 底层通过红黑树来描述，并且维护一个 ready list，将事件表中已经就绪的事件添加到这里，在使用 epoll_wait 调用时，仅观察这个 list 中有没有数据即可。
