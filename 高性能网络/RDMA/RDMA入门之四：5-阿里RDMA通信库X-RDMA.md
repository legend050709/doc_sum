...
## RDMA的优点
除了超低延迟（通常约2us）和高吞吐量之外，RDMA还支持零拷贝和内核旁路，因此可以减少与传统网络协议栈（包括上下文切换、协议处理和数据拷贝）相关的开销。
...
比如连接数和内存的占用。`Pangu`中每个`block server`上有N个线程，每个`chunk server`上有M个线程，而每两个线程之间都要建立`fullmesh`的链接，换算下来每个`chunk server`的内存占用为：`N*M*block_server_number*depth*message_size`，因此导致了高内存占用。

> 注：Pangu一个阿里云开发的高可靠、高可用、高性能的分布式文件系统，类似于Ceph。Pangu中每个服务器上有两个核心组件：block server和chunk server。
...
- 单边语义：Write/Read/Atomic这类语义不需要对端CPU参与。
...
X-RDMA与最近的研究项目完全专注于压榨原始硬件性能不同，它强调大规模生产集群的健壮性、可伸缩性和可维护性。

X-RDMA简化了编程模型，扩展了RDMA协议以实现应用程序感知，并提出了每台机器具有数千个连接的资源管理机制。它还使用内置的跟踪、调优和监视工具减少了管理和性能调优的工作量。
...
是一种在**特定约束或环境条件**下，通过优化参数、资源分配或策略，使系统或模型的性能指标达到最优的方法。这一概念广泛应用于计算机系统、机器学习、网络优化、资源管理等领域，核心目标是在满足前置条件（如硬件限制、实时负载、能耗约束等）的前提下，实现性能的极限提升。
...
**RDMA缓冲区预分配会引入大量开销**
由于RDMA操作的内存需要事先注册，这个动作是将虚地址pin在物理内存中防止page换出（即物理内存的swap），然后将页表项也发给网卡，这个动作比较耗时间。
...
这种划分类似于`MPI`的渴望和会合协议（`eager and rendezvous protocol`）。
...
### 小消息
小消息对延迟敏感，默认将4KB以内的消息称为小消息。
采用`RDMA` 的 `SEND/RECV`完成收发，两侧各只需要下发一个WR，效率比较高。

对于小消息，发送方直接向接收方发送数据，从而触发接收WR。每个数据传输只需要一个RDMA操作。但是，接收方需要预先分配足够的缓冲区。因此，为避免较多的内存占用，小消息的有效负载大小不能很大，否则它将消耗更多的缓冲区，从而导致高内存消耗。

### 大消息
大消息对吞吐敏感，大消息收发通过多轮协商（造成延迟增大）完成。

对于大消息，发送方将首先发布RDMA Send WR以唤醒接收方。接收器将根据需要准备可供RDMA使用的缓冲区。我们称这个阶段为缓冲准备阶段。之后，实际的数据传输依赖于来自发送方的RDMA写/读。在此模式下，每个数据传输至少需要两个RDMA操作。
