...
### 小结：PFC 看入口、ECN 看出口
在交换机内部，PFC 和 ECN 并不是在同一个位置工作。PFC 盯的是入口，ECN 盯的是出口。
**PFC 看的是入口缓冲区是不是顶不住。ECN 看的是出口队列有没有开始排队。**

你可以这样理解一台交换机里 RoCE 流量的路径：
```bash
1>. 包从某个入口端口进来
2>. 先被分类到某个 priority / PG（Priority Group）
3>. 暂存在入口缓冲（ingress buffer）
4>. 查表决定要从哪个出口端口出去
5>. 再被放入某个 TC（Traffic Class）/ 出口队列
6>. 等待调度发出
```
**入口这边**：问题是“我收进来的流，会不会把入口缓存撑爆？”
**出口这边**：问题是“我要发出去的流，会不会在出口排队太深？”

这就是为什么：**PG 和 PFC 天然绑在一起，TC 和 ECN 天然绑在一起**

![](attachments/Pasted%20image%2020260626162120.png)
...
![](attachments/Pasted%20image%2020260626162502.png)
...
把这个包归到一个 **switch-priority**。接着，入口侧会把这个 priority 绑定到某个 **PG（Priority Group）**。
PG 是什么？你可以先把它理解成：**入口侧按优先级划分的一组缓冲归属。**
也就是说：
- 这个包不是“直接进整个交换机缓冲区的大池子”
- 而是“先进某个入口优先级组的地盘”

### 第二步：包进入入口缓冲
这时候，包还没发出去。  它只是被收进来了。
如果后面的出口有拥塞，或者下游路径有反压，入口缓冲就会继续涨。
而 PFC 盯的，就是这一段。
...
到了出口，不同优先级 / 业务类别，通常会映射到不同 **TC（Traffic Class）**。
TC 你可以理解成：**出口调度视角下的业务通道。**
不同 TC：
- 可以有不同队列
- 可以有不同调度权重
- 可以有不同 ECN 阈值
...
而 ECN 盯的，就是这一段。因为只有到了出口，你才真正有“排队”这件事。
> **包是在入口被“收进来”的。包是在出口被“排出去”的。**
> **所以 PFC 天然是入口视角。ECN 天然是出口视角。**

### PG 为什么和 PFC 有关？
PFC 的作用是：当某个优先级对应的入口缓冲快顶不住时，通知对端先暂停发这个优先级的流量。不是“给高优先级流量加保护”。
也就是说，PFC关心的不是抽象的“业务重要性”。它关心的是：**这个优先级在入口缓冲里，是不是快把空间吃满了。**
这就决定了：**PFC 一定要跟入口侧的缓冲归属绑定。** 而入口侧缓冲归属，就是 PG。
...
而 PFC pause 发出去的时候，是在告诉对端：“你先别再往我这个优先级上打流了，我入口快撑不住了。”
你看，这个动作的视角，天然就是入口。

**为什么 lossless 通常配置在 ingress PG 上**？
因为 lossless 的核心目标不是“绝不排队”。  而是：尽量避免因为瞬时拥塞而把包丢了。
要做到这一点，就必须在入口还没溢出前，先把对端刹住。

![](attachments/Pasted%20image%2020260626162918.png)


### TC 为什么和 ECN 有关？

ECN 的本质，不是“流量分类”，ECN 不是拿来决定“这是什么流”的。而是：**当出口排队开始变深时，提前给发送端一个拥塞信号。** 关键点在“出口排队”。ECN 要回答的问题是：**哪个出口业务通道开始拥塞了？**；而这个“业务通道”，通常就是 TC。

为什么一定是出口？因为排队这件事，本质上发生在：多个流都想从同一个出口端口出去。；这个时候，出口队列长度才真正有意义。
> **ECN 不关心“你从哪进来”。ECN 关心“你从哪里出去时开始排队”。**

![](attachments/Pasted%20image%2020260626163120.png)
