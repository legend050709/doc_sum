...
**补充说明**
（1）GRH（40B）是可选的，只有当需要==跨子网路由或使用多播==功能时才需要。
（2）通过结合使用SGID、DGID、TCLASS、FLOWLABEL等字段，IBA路由器可以实现复杂的路由和QoS策略。
（3）==GRH（40B）的设计借鉴了IPv6的一些概念==
例如IPVER、TCLASS、FLOWLABEL和HOPLMT等字段，这有助于IBA网络与IP网络的互操作性。
...
==OpCode：既指定了服务类型(`[7:5]bit`, 比如RC/UD)，又指定了具体的操作以及当前包是该操作的阶段(起始、中间，结束)==。
...
2. 接收 RNR NAK： 请求方收到 RNR NAK 后，会记录下 RNR NAK 中指定的重试间隔和当前的重试计数器(3 bit)。
3. 等待重试间隔： 请求方会等待 RNR NAK 中指定的重试间隔。
4. 检查重试计数器： 如果重试计数器不为 0，则进行重试。
5. 递减重试计数器： 每次重试后，请求方会将重试计数器减 1。
6. 重试计数器为 0： 如果重试计数器减为 0，则向上报告错误，表示重试失败。
...
Message/操作完成的条件：
11. 所有包按序到达（由 PSN 保证）
12. 最后一个包的标志为 Last 或 Only
13. 接收端 MSN++
14. 完整消息被 RNIC 写入用户 buffer（对 recv 操作）
15. 若该 WQE 配置了 completion → CQE 产生
```
...
16. **链路层 (L2)：** **PFC 或 IB 链路信用** 负责**逐跳**防止**链路和交换机缓冲区溢出**，是实现无损通信（低延迟 RDMA 的基础）的关键。
    
17. **传输层流控 (L4 End-to-End)：** **RC 消息级信用流控** 负责**端到端**防止**终端主机（接收方）的 WQE 资源溢出**。
    
18. **拥塞控制 (L3/L4)：** **DCQCN/ECN** 负责**端到端**调整**发送速率**，以适应**网络拥塞**（队列排队）。


所有机制必须协同工作才能确保 RDMA 的高性能：
19. 当发送方要发送一个消耗 WQE 的请求时，它必须首先满足 **RC 消息级信用流控** (SSN≤LSN) 的要求（L4）。
20. 请求包进入网络后，在通过每个交换机时，必须遵守 **PFC 或 IB 链路层信用流控** 的要求，以确保无损传输（L2）。
21. 在整个传输过程中，如果交换机队列开始积压，**ECN** 会标记拥塞。接收端将反馈 **CNP**，使发送端降低发送**速率**（DCQCN），从而减缓所有包的流入（L3/L4）。
...
![](attachments/Pasted%20image%2020260311150550.png)
```bash
RoCE v2 RDMA Write抓包
```

![](attachments/Pasted%20image%2020260311150854.png)
```bash
RDMA SEND抓包
```
...
![](attachments/Pasted%20image%2020260311150750.png)
```bash
RDMA传输层拆包
```
...
```bash
BTH头：作为IB的基本传输层头，包含以下字段：

Opcode：既指定了服务类型(`[7:5]bit`, 比如RC/UD)，又指定了具体的操作以及当前包是该操作的阶段(起始、中间，结束);
PSN (Packet Sequence Number)(24bit)：类似于TCP头中的seq_num;
Destination QP(24bit): 类似于tcp头中的dstport，查找具体的应用，此中`dst-QPN`用于在目的节点(含有多个QP)查找具体的QP；
```
...
![](attachments/Pasted%20image%2020260311150657.png)
