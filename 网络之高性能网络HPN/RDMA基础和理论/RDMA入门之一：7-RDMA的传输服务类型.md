...
其作用类似于传统网络中UDP Socket的"目标地址+端口"，但额外包含RDMA链路层的关键参数。
...
### 不可靠：UD的包中不存在psn
`UD`由于是不可靠连接，不保证包是否达到，是否有序等，因此包中不存在`PSN`。
如果想要`UD`变得可靠：可能需要在应用层(比如：payload data)中添加`PSN`、`ACK`等可靠性机制。

## UD和数据分段、IP分片
### 为什么 RDMA UD 不支持按 MTU 拆分消息？

==UD不支持对大的消息进行MTU拆分为多个包，意味着，UD模式下 ，一个消息，就是发送一个RDMA包。因此，限制消息的大小为MTU==。

#### RC类型发送端是如何拆分大消息发送，接收端收到包是如何重组消息的？
...
#### 使用 DCT/DC（Dynamically Connected Transport）
