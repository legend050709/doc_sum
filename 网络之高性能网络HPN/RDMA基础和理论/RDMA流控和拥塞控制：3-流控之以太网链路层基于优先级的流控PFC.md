...
## PFC的引入
既然无法依赖网络设备的缓冲区，那么就需要引入拥塞控制技术（或者流控技术）来控制链路上的流量大小，减轻设备缓冲区的压力，避免丢包。拥塞控制技术旨在充分利用网络带宽容量以及充分降低传输延迟的前提之下，尽可能的保障网络不会拥塞丢包。
...
**优先级流控制 (PFC)** 被引入到==Roce==，用于**在以太网强制模拟 IB 的无损环境**。当交换机缓冲区快满时，PFC 通过发送 PAUSE 帧，使上游设备停止发送特定优先级 (Priority) 的数据，从而避免缓冲区溢出和丢包。
...
### PFC watchdog（PFC Storm Prevention）
#### PFC 死锁 导致 整个网络或部分网络的吞吐量将跌零
