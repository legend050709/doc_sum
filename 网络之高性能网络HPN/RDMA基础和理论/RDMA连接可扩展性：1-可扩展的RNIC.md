...
一方面，网络可扩展性问题来自PFC（基于优先级的流控制），这是RDMA实现无损网络结构所需的。PFC带来了诸如线路阻塞、拥塞扩展、偶尔死锁和大规模集群中PFC风暴等问题。因此，数据中心运营商倾向于将PFC配置限制在较小的网络范围内（例如，中等集群）。

另一方面，连接可扩展性问题是当连接数（即队列对（QP））超过某个小阈值（例如256）时RDMA性能急剧下降的现象。尽管商用RNIC是黑盒，但这种性能崩溃现象的根本原因被解释为连接之间的上下文切换导致的缓存未命中。
