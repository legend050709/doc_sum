...
由于incast事件和拥塞是这类集群中的常态，并且我们不确定是否会有其他供应商的错误导致PFC风暴，因此我们决定尽最大努力防止任何PFC暂停。因此，我们调整了CC算法(DCQCN)以快速降低速率（TD小），并保守地增加速率（TI大），以避免触发PFC暂停。
...
**（1）收敛慢**
对于粗粒度反馈信号，如ECN或RTT，当前的CC方案不知道增加或减少多少发送速率。因此，他们使用**启发式猜测**来进行速率更新，并尝试迭代收敛到稳定的速率分布。这种迭代方法处理大规模拥塞事件的速度很慢，如案例1所示。
