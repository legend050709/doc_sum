...
DCQCN 的核心定位： ==基于 ECN 的标记信号，动态控制每个 QP 的发送速率， 在零丢包的前提下最大化网络吞吐， 并将 PFC 的触发频率降至最低==。
...
2. 转发设备DeviceA发现端口存在拥塞，因此在转发报文时给报文携带ECN拥塞标记（ECN字段置为11），以标示网络中存在拥塞。
3. 转发设备DeviceB收到报文后，发现报文的目的地址不是本机，则不对报文进行处理，正常转发给宿端服务器。
4. 宿端服务器收到报文后，发现报文中携带ECN拥塞标记（ECN字段为11），则知道网络中存在拥塞，因此向源端服务器发送CNP拥塞通知报文，以通知源端服务器进行流量降速。
...
5. **即时惩罚 (Multiplicative Decrease):** 发送端立即将其允许的发送速率 Rallow​ 乘以一个**惩罚因子 β**（例如 β=0.5），即：
```bash
R_new​=R_old​×β
```
  
6. **CNP抑制 (CNP Suppression):** 为了避免接收端在短暂拥塞期间发送过多的CNP导致发送端过度惩罚，DCQCN通常会采用一个**时间窗口**或**CNP计数器**来抑制后续的CNP，直到速率降低生效。
    
7. **速率增加 (Additive Increase):** 在没有收到CNP（即没有拥塞）时，发送端会像传统TCP一样，**缓慢地、线性地增加**其速率，直到达到带宽限制或再次触发拥塞。
...
队列深度监控与 ECN 标记。
出端口队列深度在 Kmin 到 Kmax 之间时， 按概率对数据包 IP 头的 ECN 字段打标记（CE 位置 1）。 
超过 Kmax 后对所有包打满标记。

交换机不参与速率控制，只生成拥塞信号。

### 接收方 RNIC（Receiver）：NP

CNP 生成与发送。
收到带 ECN 标记的数据包后， 生成 CNP（Congestion Notification Packet）发回发送方。
每个 QP 每 **50 微秒**最多发送一个 CNP， 防止 CNP 报文本身占用过多带宽。

### 发送方 RNIC（Sender）：RP

速率控制的执行核心。
维护每个 QP 的发送速率， 根据 CNP 信号和计时器执行三阶段速率控制。

#### 速率控制三阶段

![](attachments/Pasted%20image%2020260509122505.png)

DCQCN 基于 ECN 的 CNP 信号， 对每个 QP 执行==降速、快速恢复、线性增加== 三阶段动态速率控制。

##### 第一阶段：降速（Rate Decrease）

每收到一个 CNP，立刻将当前速率乘以`(1 - α/2)` 进行降速。

`α` 是动态拥塞因子，取值范围 0 到 1， 反映当前路径的拥塞程度，更新公式为：
```bash
收到 CNP 时：α = (1 - g) × α + g × 1  
未收到 CNP：α = (1 - g) × α
```

`g` 为平滑系数，控制 `α` 的更新速度， 默认值通常为 1/256。
`α` 越接近 1，说明拥塞越严重， 每次 CNP 触发的降速幅度越大。

![](attachments/Pasted%20image%2020260509122612.png)


##### 第二阶段：快速恢复（Fast Recovery）

一段时间内未收到 CNP 后， 进入快速恢复阶段。
每个计时周期将当前速率向目标速率靠拢：
```bash
Current Rate = (Current Rate + Target Rate) / 2
```

快速恢复持续 **F 个计时周期**（默认值通常为 5）， 逐步将速率拉回拥塞前水平。


##### 第三阶段：加法增加（Additive Increase）

快速恢复结束后，以固定步长 RAI 线性增加速率， 直到达到链路速率上限或再次收到 CNP。
逻辑与 TCP 拥塞避免阶段类似， 但控制粒度是单个 QP，响应速度远快于 TCP。

![](attachments/Pasted%20image%2020260509122728.png)
...
==理想状态下，DCQCN 将拥塞消解在 PFC 触发之前； Incast 等极端场景下，PFC 作为最后一道防线兜底。PFC、ECN、DCQCN 三者需作为整体进行参数设计， 任何单一机制的配置失当都会影响全局表现==。
...
#### DCQCN 响应及时时
DCQCN 响应及时时，队列深度在触达 PFC 阈值之前回落：
```bash
发送速率过高  
→ 队列深度超过 Kmin  
→ ECN 开始标记  
...
#### DCQCN 响应不及时时
DCQCN 响应不及时时（典型场景：Incast）， 队列在极短时间内从空到满， ECN 标记和 CNP 往返时延来不及响应， PFC 直接触发作为最后一道防线：
```bash
Incast：大量发送方同时注入流量  
→ 队列瞬间填满  
→ ECN 来不及响应  
...
**PFC 触发次数是衡量集群网络健康状况的核心指标之一。**触发频繁，说明 DCQCN 的速率控制未能在 PFC 阈值之前 完成收敛，需要重新审视参数配置。
...
|参数|含义|配置偏差的影响|
|---|---|---|
|Kmin / Kmax|ECN 标记水位线|过高：ECN 响应滞后，PFC 频繁触发；过低：带宽利用率下降|
|g（平滑系数）|α 更新速度|过大：速率抖动剧烈；过小：对拥塞变化反应迟钝|
|F（快速恢复周期）|快速恢复持续时间|过大：速率恢复过慢；过小：拥塞未消散就开始升速|
|RAI（加法增加步长）|线性恢复速度|过大：反复触发拥塞；过小：带宽利用率恢复缓慢|
|CNP 发送间隔|CNP 生成频率上限|过大：拥塞信号稀疏；过小：CNP 占用带宽增加|

参数之间存在相互依赖， 单独调整任何一个都可能引发连锁反应。
