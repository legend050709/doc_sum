...
## 多线程下共享数据的释放问题
### 问题
比如：LB中conhash的共享调度数据（sche_data），在控制线程中生成，然后在转发线程中被使用。
sche_data 更新的时候，会在控制线程中生成新的共享调度数据（sche_data_new）;
原有的 sche_data 需要被释放掉。
这里有一个问题，就是sche_data在释放的时候，不可以有线程访问它。

### 方案一：多线程通过原子变量共享引用
每次 sche_data 在转发线程中被使用，则引用计数就会`++`；解引用就会`--`；
...
### 方案二：每个线程独立引用
由于每个线程只存在对于共享调度数据（sche_data）的引用和解引用操作，不存在其他的操作。
因为，对于共享调度数据（sche_data），可以为每个线程设置一个标记，标记为0，就是未被引用；标记为1，就是被引用了。

这样，引用技术部分就可以是线程独占，而不是共享数据引用计数。
