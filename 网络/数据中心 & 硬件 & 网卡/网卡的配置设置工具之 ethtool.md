...
![](attachments/Pasted%20image%2020241023215048.png)

**adaptive-rx**：是否启用接收路径上的自适应中断合并。

**adaptive-tx**: 是否启用发送路径上的自适应中断合并。

**rx-usecs**：设置接收（RX）路径上的中断延迟时间（微秒为单位），即在触发中断之前等待的时间。如果是为0，这意味着每个接收到的数据包都会立即触发中断，而不会等待任何时间。

**rx-frames**：设置接收（RX）路径上的中断触发阈值。产生中断前等待的帧数。
这意味着每接收到n个数据包时触发一次中断。

**irq** ：用于在网络接口已经处理中断时配置中断调节。
**rx-usecs-irq**：当主机正在处理中断时，延迟 RX 中断多少微秒。
**rx-frames-irq**：系统处理中断时，在生成 RX 中断之前要接收的最大数据帧数。

### 自适应的中断合并

```bash
ethtool -C _enp136s0_ adaptive-rx on
```

### 优缺点

中断合并（ interrupt coalescing）可以 减少网卡产生的中断数，防止中断风暴（interrupt storms）。合并调优的目的是最小化给定工作负载所需的中断数量。

在高吞吐量情况下，目标是有尽可能少的中断，同时保持高数据率。
在低延迟情况下，可以使用更多中断来快速处理流量。

**效果**
中断越少，吞吐率越高，延迟越高，CPU 利用率越低。
中断越多，吞吐率越低，延迟越低，CPU 利用率越高。
