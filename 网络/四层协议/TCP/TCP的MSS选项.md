...
![](attachments/Pasted%20image%2020231129112428.png)
...
这里怎么不考虑 IP 和 TCP 中的可能存在的 option ？！原来 RFC 规定**通告的 MSS 就是不包含option**：
```c
...
我们常常会看到一个说法，**TCP 两端在建立连接时，会将 MSS 协商为双方通告 MSS 的较小值**。这个说法其实是不准确的，RFC 879 明确指出了，没有这样的协商(negotiation)过程：
```c
...
struct tcp_sock{
    ...
    {
         // code omitted...
         u16 user_mss;    /* 用户通过 TCP_MAXSEG sockopt 设置的MSS */
...
- `rx_opt.mss_clamp`: 连接建立阶段本端计算出的 MSS。它取`user_mss`和 对端 SYN(或者 SYNACK) 报文通告的 MSS 值中的较小值，如果用户没有设置 user_mss，则就为对端报文中的 MSS 值。clamp 中文翻译为”夹钳”，我们可以理解为**生效 MSS**的最大值。
...
注：tcp 的 timestamp option正常是10B。但是由于tcp option的总长度需要是4的倍数，因此需要填充2个NOP，option一共占用了12B。
![](attachments/Pasted%20image%2020231129114054.png)
![](attachments/Pasted%20image%2020231129114158.png)
TCP头中的Data Offset（数据偏移量）
> 数据偏移量（4位）指定TCP头（包含OPtion）的长度，需要这个值是因为option字段的长度是可变的。这个字段占4bit（最多能表示15个32bit的的字，即4*15=60个字节的首部长度），因此TCP最多有60字节的首部。如果没有任选字段，正常的长度是20字节；如果有额外的TCP的option选项，还得加上option的长度。

**实际的TCP头部的长度 =  Data_Offset * 4**
因为Data Offset 表示的TCP头部的长度必须是4字节的整倍数。首先不包含option的tcp hdr长度为20，是4的倍数。因此，如果TCP Options选项不足4字节的整倍数，就需要Padding填充(即填充NOP option，每个NOP为1B)为4字节的整倍数。
 
## mss_cache的设置
`mss_cache`在 Linux 内核中表示 **TCP 连接当前生效的 MSS**。
...
而`__tcp_mtu_to_mss()`呢？它也就是传入的 pmtu 减去IP首部长度(含option)，在减去TCP首部长度(不含option)。另外，它还不能超过`rx_opt.mss_clamp`
```
...
```
### icmp差错报文设置
在连接建立之后，如果 TCP 报文超过了传输路径上某个网络设备的 mtu，且报文设置了 DF (Don’t Fragment) 标记，则该设备会反馈一个 ICMP_FRAG_NEEDED 报文，并携带支持的最大 mtu 值。原来的报文发送端收到该 ICMP 报文后，变会调整自己的 `mss_cache`
```c
...
void tcp_v4_err(struct sk_buff *icmp_skb, u32 info)
    ...
    |-- tcp_sync_mss(sk, mtu);
```
![](attachments/Pasted%20image%2020231129121031.png)
...
## 路由设置
## 内核参数设置
![](attachments/Pasted%20image%2020231129143140.png)
### ip_no_pmtu_disc
![](attachments/Pasted%20image%2020231129143816.png)

# 其他
## UDP应用收到 icmp need-frag的处理流程
```c
udp_err ---> __udp4_lib_err --> ipv4_sk_update_pmtu -->
```
![](attachments/Pasted%20image%2020231129122136.png)
![](attachments/Pasted%20image%2020231129122351.png)
# 参考
```c
