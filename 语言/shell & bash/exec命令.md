...
## exec执行命令 
### 特点
***exec是内置命令，执行命令时，不会启用新的shell进程，且exec命令后的其他命令将不再执行***

因为exec是**内建命令**，所以执行exec命令并不启动新的shell，而是用要被执行命令替换当前的shell进程，并且将老进程的环境清理掉，而且exec命令后的其它命令将不再执行。
...
```
### 注意
**注意：exec命令不是shell，因此shell脚本中定义的function和alias等，exec在执行时都找不到。**
...
因此，可以这样认为：
***exec系统调用并没有创建新的进程，只是替换了原来进程上下文的内容。原进程的代码段，数据段，堆栈段被新的进程所代替。且exec执行完之后，不会继续执行原来代码段之后的程序 ***
...
# 使用重定向方式打开文件时，可以将手动指定的文件描述符数值指定为一个 {var_name} 格式，这样 bash 会自动分配文件描述符号，并将文件描述符数值保存到 bash 变量 var_name 中。
exec {tmp_alog}> /tmp/a.log
```

### 作用
在脚本中使用exec重定向比较有用，可以统一将输出进行重定向，包含shell的子进程，而不用在每一条命令上设置重定向，减少很多键盘工作。

注：正常重定向直接操作的是文件。但是shell的子进程以及孙子进程可能继承的是fd，而不是文件的操作。在子进程以及孙子进程中可能无法重定向到自定的文件中。因此，通过fd和指定的文件操作，然后将输入内容重定向到指定的fd，这样子进程以及孙子进程也可以通过继承fd的方式，重定向到指定的文件中。

#### exec 输入重定向
exec < filename：
会将stdin重定向到文件中. 从这句开始, 所有的stdin就都来自于这个文件了, 而不是标准输入(通常都是键盘输入). 这样就提供了一种按行读取文件的方法, 并且可以使用sed和/或awk来对每一行进行分析.
```bash
#!/bin/bash
# 使用'exec'重定向stdin.
...
### 特点
exec命令对文件描述符操作时，对整个shell文件以及shell子进程的重定向生效。
其不会替换当前shell，且操作完成后还会继续执行后面的命令。
