...
**SSN 的概念**： 请求方逻辑上为每个 WQE 分配一个 SSN，与响应方返回的 MSN 一一对应。这只是一个逻辑概念，实际实现中不一定需要完全按照此方式实现。
...
##### 端到端（消息级）流控
端到端credits在 AETH 中以编码的 5 位字段的形式携带。
响应方可以通过使用非应答ACK包异步地向请求者发送credits。非应答ACK包指的是重新发送最近发送的ACK包。
LSN (limited SN)； 
![](attachments/Pasted%20image%2020250804202649.png)
...
**丢失数据检测（PSN）**： 发送方和接收方通过序列号等机制来检测丢失的数据包。
    
**超时重传机制**： 发送方在超时后重传未被确认的数据包。
...
BTH 中的操作码用于指示数据包的类型和在消息中的位置（例如，First、Middle、Last、Only）。以下表格总结了根据前一个数据包的操作码，当前数据包的有效操作码：
