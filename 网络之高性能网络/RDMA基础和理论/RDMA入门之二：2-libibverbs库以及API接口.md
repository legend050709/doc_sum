...
`IBV_SEND_SOLICITED` 是发送端给接收端打的一个“标记”，告诉接收端：“如果接收端配置了 `solicited_only` 模式，那么处理我这个消息时，请在接收端生成一个完成事件（WC）”。它是发送端控制接收端在特定配置下是否生成完成事件的“请求开关”。

##### 接收端为什么需要设置`IBV_SEND_SOLICITED` 标志

**（一）如果服务端不设置 `solicited_only=1`（即：使用默认的 `solicited_only=0` 或等效配置）**：
...
**(2.2) 带立即数的RDMA写入：** 
如果客户端执行了 `IBV_WR_RDMA_WRITE_WITH_IMM` 操作，服务端需要预先为接收立即数（而不是数据本身）发布Recv WR。当这种写入操作完成时，服务端会收到一个包含立即数的接收完成WC（`opcode` 为 `IBV_WC_RECV_RDMA_WITH_IMM`）。**这完全独立于 `SOLICITED` 标志。**
...
即：必须确保在创建接收端 CQ 时设置了启用 Solicited-Only 模式的标志。仅仅发送端设置 `IBV_SEND_SOLICITED` 而接收端 CQ 未配置 `solicited_only=1` 是无效的！==如果接收端 CQ 是默认配置 (`solicited_only=0`)，那么所有接收到的消息都会生成 WC，发送端的 `SOLICITED` 标志就没有实际作用（虽然包里有标记，但接收端无视这个标记）==。

##### `IBV_SEND_SOLICITED`匹配的操作类型
```bash
...
控制路径的API调用（主要是配置硬件资源等）需要通过内核来执行，会导致上下文切换；
但是数据路径的API调用不会导致上下文切换。
...
```bash
WQ events:
    IBV_EVENT_WQ_FATAL:  Error occurred on a WQ and it transitioned to
```
#### CQ相关事件
...
`ibv_get_async_event()` 是一个阻塞函数。如果多个线程同时调用此函数，那么当发生异步事件时，只有一个线程会接收到该事件，并且无法预测哪个线程会接收到它。
